...
int main(int argc, char **argv)
{
    ...
    Network * net2 = Network::CreateNetworkFE( topology_file, backend_exe, &dummy_argv );
    if( net2->has_Error() ) {
    ...
    for( unsigned int i=0; i<num_iters; i++ ){
        ...
        if( recv_val != expected_val ){
            fprintf(stderr, "[1] Iteration %d: Failure! recv_val(%d) != %d*%d*%d=%d (send_val*i*num_backends)\n",
                    i, recv_val, send_val, i, num_backends, expected_val );
        }
        else{
            fprintf(stdout, "[1] Iteration %d: Success! recv_val(%d) == %d*%d*%d=%d (send_val*i*num_backends)\n",
                    i, recv_val, send_val, i, num_backends, expected_val );
        }
        ...
        if( recv_val != expected_val ){
            fprintf(stderr, "[2] Iteration %d: Failure! recv_val(%d) != %d*%d*%d=%d (send_val*i*num_backends)\n",
                    i, recv_val, send_val, i, num_backends, expected_val );
        }
        else{
            fprintf(stdout, "[2] Iteration %d: Success! recv_val(%d) == %d*%d*%d=%d (send_val*i*num_backends)\n",
                    i, recv_val, send_val, i, num_backends, expected_val );
        }
    }
    delete add_stream;
    ...
    Stream * ctl_stream2 = net2->new_Stream( comm_BC2, TFILTER_MAX,
                                             SFILTER_WAITFORALL );
    if(ctl_stream2->send(PROT_EXIT, "") == -1){
