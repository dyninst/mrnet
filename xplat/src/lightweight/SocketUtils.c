...
static bool_t SetCloseOnExec( XPlat_Socket sock )
{
    ...
    if( fdflag == -1 ) {
        ...
        xplat_dbg( 1, fprintf(stderr, "XPlat_SocketUtils_SetCloseOnExec - "
                              "failed to get flags\n") );     
	return false;
    }
    else {
                                  ...
                                  unsigned num_retry )
{
    ...
    if( _sock <= 0 ) {
        ...
        xplat_dbg( 5, fprintf(stderr, "XPlat_SocketUtils_Connect - "
                              "socket() => %d\n", _sock) );
    }
                                          ...
                                          bool_t nonblock )
{
    ...
    if( -1 == _sock ) {
        err = XPlat_NetUtils_GetLastError();
        err_str = XPlat_Error_GetErrorString(err);
        xplat_dbg( 1, fprintf(stderr, "XPlat_SocketUtils_CreateListening - "
                              "socket() failed with '%s'\n", err_str) );
        return false;
    }

    xplat_dbg( 3, fprintf(stderr, "XPlat_SocketUtils_CreateListening - "
                          "sock:%d, port:%d\n",
                          _sock, _port) );
                                           ...
                                           bool_t nonblock )
{
    ...
    XPlat_Socket connection = InvalidSocket;
    const char* err_str;
    fd_set readfds;
    struct timeval tv;

    xplat_dbg( 3, fprintf(stderr, "XPlat_SocketUtils_AcceptConnection - "
    ...
    do {
        if( timeout_sec > 0 ) {
            ...
            if( retval == 0 ) { // timed-out
                ...
                err_str = XPlat_Error_GetErrorString(err);
                xplat_dbg( 1, fprintf(stderr, "XPlat_SocketUtils_AcceptConnection - "
                                  ...
                                  XPlat_Port* port )
{
    ...
    if( -1 == getsockname(sock, (struct sockaddr*) &local_addr, &sockaddr_len) ) {
        err = XPlat_NetUtils_GetLastError();
        err_str = XPlat_Error_GetErrorString(err);
        xplat_dbg( 1, fprintf(stderr, "XPlat_SocketUtils_getPort - " 
