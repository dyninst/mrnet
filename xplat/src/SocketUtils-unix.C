...
ssize_t Send( XPlat_Socket s, NCBuf* ncbufs, unsigned int nBufs )
{
    ...
    while( currBufNdx < nBufs ) {
        ...
        for( i = 0; (i < currIovLen) && (currBufNdx < nBufs); i++ ) {
            ...
            // fprintf(stdout, "XPlat DEBUG: Send - currBuf->len = %zd\n", len);
        ...
        if( wret < 0 ) {
            ...
            fprintf(stderr, "Error: XPlat::SocketUtils::Send - writev() : %s\n", 
                    strerror(err));
            ret = wret;
            break; // out of while
        }
        ...
        if( wret != numBytesToSend ) {
            ...
            // fprintf(stdout, "XPlat DEBUG: Send - writev wrote %zd of %zd bytes\n", wret, numBytesToSend);
            ...
            for( i = 0; i < numIovBufs; i++ ) {
                ...
                if( running_total == wret ) { 
                    ...
                    if( wret < 0 ) {
                        free( currIov );
                        fprintf(stderr, "Warning: XPlat::SocketUtils::Send - wrote %zd of %zd bytes\n", 
                                ret, numBytesToSend);
            ...
            if( i < numIovBufs ) {
                for (;i < numIovBufs; i++) { 
                    ...
                    if( wret < 0 ) {
                        free( currIov );
                        fprintf(stderr, "Error: XPlat::SocketUtils::Send - fallback send() : %s\n", 
                                strerror(err));
                        return ret;
                    }   
                    ret += wret; 
...
ssize_t Recv( XPlat_Socket s, NCBuf* ncbufs, unsigned int nBufs )
{
    ...
    while( nBufsLeftToRecv > 0 ) {
        ...
        if( rret < 0 ) {
            perror( "XPlat::SocketUtils::Recv - recvmsg()");
            ret = rret;
#if ! (defined(os_solaris) && defined(compiler_sun))
            int err = msg.msg_flags;
            fprintf(stderr, "Warning: XPlat::SocketUtils::Recv error msg_flags=%x\n", err);
#endif
            break; // out of while
        }
        ...
        if( rret != numBytesToRecv ) {
            ...
            for( i = 0; i < nBufsToRecv; i++ ) {
                ...
                if( running_total == rret ) { 
                    ...
                    if( rret < 0 ) {
                        delete[] msg.msg_iov;
                        fprintf(stderr, "Warning: XPlat::SocketUtils::Recv - got %zd of %zd bytes\n", 
                                ret, numBytesToRecv);
                        return ret;
                    }
                    ret += rret;
...
ssize_t send( XPlat_Socket s, const void *buf, size_t count )
{
    ...
    //fprintf(stderr, "XPlat DEBUG: send - writing %zd bytes to fd=%d)\n", count, s);
    ...
    while( bytes_written != count ) {
        ...
        if( ret == -1 ) {
            if( (err == EINTR) || (err == EAGAIN) || (err == EWOULDBLOCK) ) {
                ...
                fprintf(stderr,
                        "Warning: XPlat::SocketUtils::send - premature return from send(). "
            ...
            if( bytes_written < count ) {
                ...
                //fprintf(stderr, "XPlat DEBUG: send - returning %zd\n", bytes_written);
                return bytes_written;
            }
        }
    }
...
ssize_t recv( XPlat_Socket s, void *buf, size_t count )
{
    ...
    while( bytes_recvd != count ) {
        ...
        if( ret == -1 ) {
            ...
            //fprintf(stderr, "XPlat DEBUG: recv - recv() returned 0 (peer likely gone)\n");
            return -1;
        }
        else {
            bytes_recvd += ret;
            if( bytes_recvd < count ) {
                ...
                //fprintf(stderr, "XPlat DEBUG: recv - returning %zd\n", bytes_recvd);
                return bytes_recvd;
            }
        }
    }
